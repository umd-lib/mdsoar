...
//                "AND resource_id = ? AND ((status != ? AND status != ?) OR status IS NULL)";
    ...
    protected void saveDOIToObject(Context context, DSpaceObject dso, String doi)
        ...
        // UMD Customization
        // Note: MD-SOAR saves the DOI using the "doi:" scheme,
        // via "doiService.formatIdentifier(doi)", while DRUM uses the URL scheme,
        // via "doiService.DOIToExternalForm(doi)"
        //
        // In both DRUM and MD-SOAR, the DOI is stored in "dc.identifier"
        // element, not the DSpace standard "dc.identifier.uri" element
