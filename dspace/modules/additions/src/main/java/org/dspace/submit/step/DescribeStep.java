...
public class DescribeStep extends AbstractProcessingStep {
    ...
    public int doProcessing(Context context, HttpServletRequest request, HttpServletResponse response,
        ...
        if (!request.getParameterNames().hasMoreElements()) {
            // In case of an empty request do NOT just remove all metadata, just return to
            // the submission page
        ...
        DCInput[] inputs = null;
        try {
            inputs = inputsReader.getInputs(c.getHandle()).getPageRows(currentPage - 1,
                    subInfo.getSubmissionItem().hasMultipleTitles(), subInfo.getSubmissionItem().isPublishedBefore());
        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            // Allow the clearing out of the metadata defined for other document types,
            // provided it can change anytime

            if (!inputs[i].isVisible(subInfo.isInWorkflow() ? DCInput.WORKFLOW_SCOPE : DCInput.SUBMISSION_SCOPE)) {
                continue;
            }
            if (inputs[i].getInputType().equals("qualdrop_value")) {
                ...
                for (int j = 0; j < pairs.size(); j += 2) {
                    ...
                    itemService.clearMetadata(context, item, inputs[i].getSchema(), inputs[i].getElement(), qualifier,
                            Item.ANY);
                }
            } else {
        ...
        for (int j = 0; j < inputs.length; j++) {
            ...
            if (inputType.equals("name")) {
                readNames(context, request, item, schema, element, qualifier, inputs[j].getRepeatable());
            ...
            // always rendered as a dropdown menu
                ...
                readSeriesNumbers(context, request, item, schema, element, qualifier, inputs[j].getRepeatable());
            } else if (inputType.equals("qualdrop_value")) {
                List<String> quals = getRepeatedParameter(request, schema + "_" + element,
                        schema + "_" + element + "_qualifier");
                List<String> vals = getRepeatedParameter(request, schema + "_" + element,
                        schema + "_" + element + "_value");
                for (int z = 0; z < vals.size(); z++) {
                    ...
                    if (!buttonPressed.equals("submit_" + schema + "_" + element + "_remove_" + z)
                            && !thisVal.equals("")) {
        ...
        if (buttonPressed.equals(NEXT_BUTTON) || buttonPressed.startsWith(PROGRESS_BAR_PREFIX)
                || buttonPressed.equals(PREVIOUS_BUTTON) || buttonPressed.equals(CANCEL_BUTTON)) {
            for (int i = 0; i < inputs.length; i++) {
                // Do not check the required attribute if it is not visible or not allowed for
                // the document type
                ...
                if ((inputs[i].isRequired() && values.size() == 0) && inputs[i]
                        .isVisible(subInfo.isInWorkflow() ? DCInput.WORKFLOW_SCOPE : DCInput.SUBMISSION_SCOPE)) {
    ...
    /**
     * Set relevant metadata fields in an item from name values in the form. Some
     * fields are repeatable in the form. If this is the case, and the field is
     * "dc.contributor.author", the names in the request will be from the fields as
     * follows:
     *
     ...
     *
     * dc_contributor_last -> last name of first contributor dc_contributor_first ->
     * first name(s) of first contributor
     *
     * If the parameter "submit_dc_contributor_author_remove_n" is set, that value
     * is removed.
     *
     ...
     *
     * The values will be put in separate DCValues, in the form "last name, first
     * name(s)", ordered as they appear in the list. These will replace any existing
     * values.
     *
     ...
     */
    protected void readNames(Context context, HttpServletRequest request, Item item, String schema, String element,
            String qualifier, boolean repeated) throws SQLException {
        ...
        if (repeated) {
            firsts = getRepeatedParameter(request, metadataField, metadataField + "_first");
            lasts = getRepeatedParameter(request, metadataField, metadataField + "_last");
            ...
            if ((l != null) && !((l.trim()).equals(""))) {
                ...
                if (isAuthorityControlled) {
                    ...
                    if (metadataAuthorityService.isAuthorityRequired(fieldKey)
                        ...
                        log.warn("Skipping value of " + metadataField
                                + " because the required Authority key is missing or empty.");
                        ...
                        itemService.addMetadata(context, item, schema, element, qualifier, null,
                                ...
                                (sconf != null && sconf.length() > 0) ? Choices.getConfidenceValue(sconf)
                                        : Choices.CF_ACCEPTED);
                    }
                } else {
                    itemService.addMetadata(context, item, schema, element, qualifier, null,
    ...
    /**
     * Fill out an item's metadata values from a plain standard text field. If the
     ...
     *
     * The values will be put in separate DCValues, ordered as they appear in the
     * list. These will replace any existing values.
     *
     * @param request        the request object
     * @param item           the item to update
     ...
     * @param hasLanguageTag to check if the field has a language tag
     */
    protected void readText(Context context, HttpServletRequest request, Item item, String schema, String element,
            String qualifier, boolean repeated, String lang, boolean hasLanguageTag) throws SQLException {
            ...
            if ((s != null) && !s.equals("")) {
                ...
                if (isAuthorityControlled) {
                    ...
                    if (metadataAuthorityService.isAuthorityRequired(fieldKey)
                        ...
                        itemService.addMetadata(context, item, schema, element, qualifier, lang, s, authKey,
                                (sconf != null && sconf.length() > 0) ? Choices.getConfidenceValue(sconf)
                                        : Choices.CF_ACCEPTED);
                    }
                } else {
    ...
    /**
     * Fill out a metadata date field with the value from a form. The date is taken
     ...
     */
    protected void readDate(Context context, HttpServletRequest request, Item item, String schema, String element,
     ...
     * form. Some fields are repeatable in the form. If this is the case, and the
     ...
     * @param repeated  set to true if the field is repeatable on the form
     */
    protected void readSeriesNumbers(Context context, HttpServletRequest request, Item item, String schema,
        ...
        if (repeated) {
            series = getRepeatedParameter(request, metadataField, metadataField + "_series");
            numbers = getRepeatedParameter(request, metadataField, metadataField + "_number");
     ...
     * values in the form of parameters "foo", "foo_1", "foo_2", etc. are returned.
     * <P>
    ...
    protected List<String> getRepeatedParameter(HttpServletRequest request, String metadataField, String param) {
        ...
        log.debug("getRepeatedParameter: metadataField=" + metadataField + " param=" + metadataField
    ...
    /**
     * This Methode has the same function as the getRepeatedParameter. For repeated
     * values with language tag their indices are needed to properly identify their
     * language tag
     * 
     * @param request       the HTTP request containing the form information
     * @param metadataField the metadata field which can store repeated values
    ...
    protected TreeMap<Integer, String> getRepeatedParameterWithTheirIndices(HttpServletRequest request,
        ...
        log.debug("getRepeatedParameterWithTheirIndices: metadataField=" + metadataField + " param=" + metadataField
                + ", return count = " + vals.size());
